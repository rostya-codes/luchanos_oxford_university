...
# Fixture — підготовка залежностей для тестів.
...
async def _get_test_db():
    try:
        ...
        test_engine = create_async_engine(
            settings.TEST_DATABASE_URL, echo=True
        )

        # create session for the interation with database
        test_async_session = sessionmaker(
            test_engine, expire_on_commit=True, class_=AsyncSession
        )
...
async def create_user_in_database(async_pool):
    ...
    async def create_user_in_database(
        ...
        async with async_pool.asquire() as connection:
